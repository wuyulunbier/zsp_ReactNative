...
export default class Business extends Component {
    ...
    componentDidMount() {
        ...
        //埋点的设计  进入新品页/进入小额页/进入大额页/进入低门槛页
        let software_platform = Platform.OS === 'ios' ? 'ios' : Platform.OS === 'android' ? 'android' : 'h5';

        var actString = "";
        if (this.state.type === '小额') {
            actString = '小额'
        } else if (this.state.type === '新品') {
            actString = '新品'
        } else if (this.state.type === '低门槛') {
            actString = '低门槛'
        } else if (this.state.type === '大额') {
            actString = '大额'
        }
    ...
    _onPressShowItem = () => {
        this.props.navigation.navigate('Business', { title: '小额速贷', type: 'small' });
    }
    ...
    _getHotList() {
        ...
        let software_platform = Platform.OS === 'ios' ? 'ios' : Platform.OS === 'android' ? 'android' : 'h5';
        Http.get(API.testURL + 'product' + '?' + 'type=' + this.state.type).then((res) => {
            setTimeout(() => {
                this.setState({
                    refreshing: false,
                });
    ...
    _onItemClick(item) {
        ...
        if (url.startsWith('https') || url.startsWith('http')) {
            if (url.indexOf('http') > -1 || url.indexOf('https') > -1) {
                this.props.navigation.navigate('ShowWeb', { title: item.name, url: item.target });
            }
        }
        ...
        //  this.props.navigation.navigate('LoanDetail', {title: '贷款详情', id:item.id,source:this.state.type,productname:item.name,url: 'https://wdzx.bilibilidata.cn/mini/h5/index.html'});
